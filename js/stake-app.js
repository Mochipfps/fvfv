...
(function () {
  ...
  function setCount() {
    ...
    if (b) {
      ...
      b.textContent = n > 0 ? ('STAKE ' + n + ' NFT' + (n > 1 ? 'S' : '')) : 'SELECT XCOPUNKS TO STAKE';
  ...
  /* Push-based staking: the OWNER transfers each NFT directly into the staking
     contract, whose ERC721 receiver hook records the stake. No approve(), no
     separate stake() call. ERC721 safeTransferFrom is one token per tx. */
  async function doStake() {
    ...
    try {
      ...
      if (!enabled || !glob) { status('STAKING NOT OPEN YET', 'err'); setCount(); return; }
      if (!(await X.networkOk())) { status('WRONG NETWORK — SWITCH TO ROBINHOOD CHAIN', 'err'); setCount(); return; }

      var nft = X.nftWrite(), me = X.account();
      for (var i = 0; i < ids.length; i++) {
        var id = ids[i], n = (i + 1) + ' OF ' + ids.length;
        try {
          status('STAKING XCO #' + id + ' (' + n + ') — CONFIRM IN YOUR WALLET');
          var tx = await nft['safeTransferFrom(address,address,uint256)'](
            me, X.STAKING_ADDR, ethers.BigNumber.from(id)
          );
          status('STAKING XCO #' + id + ' (' + n + ') — WAITING FOR CONFIRMATION');
          await tx.wait();
          // confirm from the contract, never assume
          var ok = await X.readStaking('isStaked', [id]);
          if (ok) { done.push(id); delete selected[id]; }
          else { failed.push(id); }
        } catch (e) {
          failed.push(id);
          status('XCO #' + id + ': ' + X.friendly(e), 'err');
          if (/ACTION_REJECTED|User denied|user rejected/i.test(JSON.stringify(e && (e.error || e.message || e) || ''))) break;
        }
      }
      if (done.length && !failed.length) status('STAKE CONFIRMED — ' + done.length + ' XCOPUNKS STAKED', 'ok');
      else if (done.length && failed.length) status('STAKED: #' + done.join(', #') + '  ·  NOT STAKED: #' + failed.join(', #'), 'err');
      await loadOwned();
    } catch (e) {
  ...
  async function initVault() {
    ...
    // Mint Fee — GROSS collected (never the current balance; ETH was spent acquiring $XCPN)
    try {
      var gross = null, verified = false;
      var cfg = window.XCP_GROSS_MINT_FEE_ETH;
      if (cfg !== undefined && cfg !== null && cfg !== '') {
        gross = ethers.utils.parseEther(String(cfg)); verified = true;   // owner-verified gross
      } else {
        gross = await inboundExternalEth(X.MINT_FEE_WALLET);             // measured on-chain
      }
      txt('vMintTotal', X.fmtEth(gross) + ' ETH');
      txt('vMintStake', X.fmtEth(X.pct(gross, 90, 100)) + ' ETH \u2192 $XCPN');
      txt('vMintBuild', X.fmtEth(X.pct(gross, 10, 100)) + ' ETH');
      var lbl = $('vMintLabel'), note = $('vMintNote');
      if (verified) {
        if (lbl) lbl.textContent = 'GROSS MINT FEE COLLECTED (100%)';
        if (note) note.textContent = 'The 90% staker allocation is distributed as $XCPN, acquired with the collected Mint Fee ETH. Figures are the gross Mint Fee collected, not the wallet\u2019s remaining balance.';
      } else {
        if (lbl) lbl.textContent = 'MINT FEE RECEIVED \u00b7 DIRECT TRANSFERS (100%)';
        if (note) note.textContent = 'Measured from direct inbound transfers to the Mint Fee wallet. Contract-routed payments are not included in this on-chain total, so the gross Mint Fee may be higher. The 90% staker allocation is distributed as $XCPN.';
      }
    } catch (e) {
      var un = String(e && e.message) === 'RPC_UNSUPPORTED';
      txt('vMintTotal', un ? 'PENDING VERIFICATION' : 'NETWORK BUSY');
      txt('vMintStake', '—'); txt('vMintBuild', '—');
      var n2 = $('vMintNote');
      if (n2 && un) n2.textContent = 'The gross Mint Fee total cannot be read from this network automatically. It will be shown once the verified figure is published.';
    }
    ...
    // $XCPN reward pool — real token balances (staking contract + mint fee wallet)
    try {
      var XCPN = '0xc1dba4c70d69296ede3a7e35d613374b806c9676';
      var tok = XCPN;
      try {
        var onchain = await X.readStaking('xcpnToken', []);
        if (onchain && !/^0x0{40}$/i.test(onchain)) tok = onchain;
      } catch (e) {}
      var res = await X.withProvider(function (p) {
        var t = new ethers.Contract(tok, X.ERC20_ABI, p);
        return Promise.all([t.balanceOf(X.STAKING_ADDR), t.balanceOf(X.MINT_FEE_WALLET), t.decimals()]);
      });
      var pool = res[0].add(res[1]);
      txt('vXcpnPool', X.fmtUnits(pool, res[2]) + ' XCPN');
  ...
  /* Total ETH received by the Mint Fee wallet from external transfers.
     NOTE: Robinhood Chain only supports the 'external' category, so contract-
     initiated (internal) value transfers are invisible here. Returns
     {total, partial} so callers can label the figure honestly. */
  async function inboundExternalEth(addr) {
    var total = ethers.BigNumber.from(0), pageKey = null, guard = 0;
    do {
      var body = {
      ...
      var j = null, sent = false;
      for (var i = 0; i < X.endpoints().length; i++) {
        try {
          var r = await fetch(X.endpointAt(i), {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body)
          });
          if (!r.ok) continue;
          j = await r.json();
          // deterministic invalid-params / unsupported method: fail fast, don't cycle endpoints
          if (j && j.error) {
            var code = j.error.code;
            if (code === -32602 || code === -32601) throw new Error('RPC_UNSUPPORTED');
            continue;
          }
          if (j && j.result) { sent = true; break; }
        } catch (e) {
          if (String(e && e.message) === 'RPC_UNSUPPORTED') throw e;
        }
      }
      if (!sent) throw new Error('RPC_ALL_FAILED');
      (j.result.transfers || []).forEach(function (t) {
        if (t.asset && t.asset !== 'ETH') return;
        var raw = t.rawContract && t.rawContract.value;
        if (raw) { try { total = total.add(ethers.BigNumber.from(raw)); } catch (e) {} }
      });
